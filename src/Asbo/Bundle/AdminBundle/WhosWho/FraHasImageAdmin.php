<?php

/*
 * This file is part of the ASBO package.
 *
 * (c) De Ron Malian <deronmalian@gmail.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

namespace Asbo\Bundle\AdminBundle\WhosWho;

use Asbo\Bundle\WhosWhoBundle\Form\EventListener\PrincipalResourceListener;
use Asbo\Bundle\WhosWhoBundle\Util\AnnoManipulator;
use Sonata\AdminBundle\Admin\Admin;
use Sonata\AdminBundle\Datagrid\ListMapper;
use Sonata\AdminBundle\Datagrid\DatagridMapper;
use Sonata\AdminBundle\Form\FormMapper;

/**
 * FraHasImage admin for SonataAdminBundle.
 *
 * @author De Ron Malian <deronmalian@gmail.com>
 */
class FraHasImageAdmin extends Admin
{
    /**
     * {@inheritdoc}
     */
    protected $parentAssociationMapping = 'fra';

    /**
     * {@inheritDoc}
     */
    protected function configureFormFields(FormMapper $formMapper)
    {
        $formMapper
            ->add('image', 'sonata_type_model_list', [],
                [
                    'link_parameters' => [
                            'context' => 'whoswho',
                        ],
                ]
            )
            ->add('anno', 'asbo_whoswho_anno')
        ;

        $formMapper->getFormBuilder()->addEventSubscriber(new PrincipalResourceListener('image'));

        if (!$this->isChild()) {
            $formMapper->add('fra', 'sonata_type_model_list');
        }
    }

    /**
     * {@inheritDoc}
     */
    protected function configureDatagridFilters(DatagridMapper $datagridMapper)
    {
        $datagridMapper
            ->add('fra')
            ->add('anno', null,
                [
                    'field_type' => 'choice',
                    'field_options' => [
                        'choices' => AnnoManipulator::getAnnos(),
                    ],
                ]
            )
        ;
    }

    /**
     * {@inheritDoc}
     */
    protected function configureListFields(ListMapper $listMapper)
    {
        $listMapper
            ->add('custom', 'string',
                [
                    'template' => 'AsboAdminBundle:WhosWho:list_fra_has_image.html.twig',
                ]
            )
            ->add('anno')
            ->add('_action', 'actions',
                [
                    'actions' => [
                        'edit' => [],
                        'delete' => []
                    ],
                ]
            );
        ;

        if (!$this->isChild()) {
            $listMapper->add('fra', 'sonata_type_model_list');
        }
    }
}
