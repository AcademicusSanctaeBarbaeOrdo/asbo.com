...
namespace Asbo\Bundle\WhosWhoBundle\Util;
...
class AnnoManipulator
{
    ...
    public static function getAnnoByDate($date)
    {
        ...
        if ($date < self::getFondationDate()) {
            throw new \InvalidArgumentException(
                sprintf('The date must be upper than %s.', self::getFondationDate()->format('d-m-Y'))
            );
        }

        if ($date > new DateTime()) {
            throw new \InvalidArgumentException(
                sprintf('The date must be lower than today.')
            );
        }

        if ($date < new DateTime(self::END_ANNO_0)) {
            return 0;
        }
    ...
    public static function getDateByAnno($anno)
    {
        if (!self::isValid($anno)) {
            throw new \InvalidArgumentException(
                sprintf('The anno must be valid. %d given.', $anno)
            );
        }
