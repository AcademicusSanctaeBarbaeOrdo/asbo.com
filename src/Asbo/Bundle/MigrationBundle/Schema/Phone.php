...
class Phone extends AbstractSchema implements OrderedFixtureInterface
{
    /**
     * {@inhertidoc}
     */
    ...
    protected function setNormalizers(OptionsResolverInterface $resolver)
    {
        $resolver->setNormalizers([
                'numero' => function (Options $options, $value) {
                    return str_replace(' ', '', $value);
                },
                'type' => function (Options $options, $value) {
                    switch ($value) {
                        case '0':
                            return PhoneTypes::PRIVEE;
                        case '1':
                            return PhoneTypes::PARENTS;
                        case '2':
                            return PhoneTypes::KOT;
                        case '3':
                            return PhoneTypes::BOULOT;
                        case '4':
                            return PhoneTypes::FAX;
                        case '5':
                            return PhoneTypes::GSM;
                        default :
                ...
                'country' => function (Options $options, $value) {
                    $number = $options['numero'];

                    foreach ($this->getCountry() as $code => $infos) {
                        foreach ($infos as $type => $info) {
                            if ($type == 'dial_code') {
                                if (strpos($number, '+'.$info)) {
                                    return $code;
                                }
                            }
                        }
